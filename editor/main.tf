...
#   }
  provisioner "local-exec" {
    command = "gcloud builds submit --tag gcr.io/serious-dialect-367517/renderer"
  }
}
  ...
  template {
    spec {
      containers {
        ...
        image = "gcr.io/serious-dialect-367517/renderer"
      }
...
resource "null_resource" "editor" {
  provisioner "local-exec" {
    command = "gcloud builds submit --tag gcr.io/serious-dialect-367517/editor"
  }
}
  ...
  template {
    spec {
      containers {
        ...
        image = "gcr.io/serious-dialect-367517/editor"
        env {
